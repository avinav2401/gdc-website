          ...
          <div className="absolute inset-0 bg-[#07080D]/95 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center gap-3 p-3 z-20">
            ...
            <div className="flex items-center gap-2">
              ...
              <a 
                ...
                className="p-2 bg-[#00F2FE] text-black border border-white hover:bg-[#FF007F] hover:text-white transition-colors shadow-[2px_2px_0px_#FF007F]"
                title="LinkedIn"
              >
...
export default function TeamPage() {
  const facultyMembers = team.filter((m) => m.team === "faculty");
  const level1GuildMasters = team.filter((m) => m.level === 1);
  const level2GuildMasters = team.filter((m) => m.level === 2);
  const level3Operatives = team.filter((m) => m.level === 3);
  const level4Initiates = team.filter((m) => m.level === 4);

  // Gallery slider state & active group photo paths (Store your photos in public/gallery/)
  const [currentSlide, setCurrentSlide] = useState(0);
  const galleryImages = [
    "/gallery/squad-1.jpeg",
    "/gallery/squad-2.jpeg",
    "/gallery/squad-3.jpeg",
  ];
        ...
        <section className="relative">
          ...
          <div className="space-y-16 relative before:absolute before:inset-0 before:left-6 md:before:left-1/2 before:-translate-x-1/2 before:w-1 before:bg-gradient-to-b before:from-[#00F2FE] before:via-[#FF007F] before:to-[#FF9F43]">
            ...
            <div className="relative">
              ...
              <div className={level4Initiates.length === 1 ? "flex justify-center" : "grid gap-6 md:grid-cols-2 max-w-4xl mx-auto items-stretch justify-items-center"}>
